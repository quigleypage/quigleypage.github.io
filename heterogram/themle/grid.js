var targetWordList = ['alien','super','movie','blade','guard','world','logan','giant','morph','tigra','havok','stick','talos','thena','shuri','grets','taser','triad','demon','storm','rogue','thing','beast','cable','angel','clint','realm','earth','namor','surge','cameo','clone','force','space','hydra','stark','limbo','ghost','magic','venom','nexus','phase','psion','stone','power','bruce','bucky','miles','agent'];
var dice = Math.floor(Math.random() * targetWordList.length);
var targetWord = targetWordList[dice];
...
function keyPress(selectedLetter){
    ...
    if(nextSpace < wordLength*rowTracker && selectedLetter != "backspace" && selectedLetter != "enter" && won == false){
        ...
        if(dictionaryArray.includes(currentGuess) || targetWordList.includes(currentGuess)){ // if the current guess is a valid word in the dictionary
